...
void Lewitacja_output(void)
{
  ...
  if (rtmIsMajorTimeStep(Lewitacja_M)) {
    ...
    Lewitacja_B.Control = Lewitacja_P.Control_Value;

    /* Level2 S-Function Block: '<S1>/Analog Inputs' (ML_AnalogInputs) */
    {
      SimStruct *rts = Lewitacja_M->childSfunctions[0];
      sfcnOutputs(rts, 1);
    }

    /* Lookup: '<S1>/Position scaling [V] to [m]' */
   ...
   *  TransferFcn: '<S1>/Position Filter'
   */
   ...
   *  TransferFcn: '<S1>/Velocity Filter'
   */
  ...
  if (rtmIsMajorTimeStep(Lewitacja_M)) {
    ...
    if (Lewitacja_B.Control > Lewitacja_P.Saturation_UpperSat) {
      Lewitacja_B.Saturation = Lewitacja_P.Saturation_UpperSat;
    } else if (Lewitacja_B.Control < Lewitacja_P.Saturation_LowerSat) {
      Lewitacja_B.Saturation = Lewitacja_P.Saturation_LowerSat;
    } else {
      Lewitacja_B.Saturation = Lewitacja_B.Control;
    }
   ...
   *  TransferFcn: '<S1>/Current Filter1'
   */
  if (Lewitacja_P.ManualSwitch3_CurrentSetting == 1) {
    Lewitacja_B.ManualSwitch3 = Lewitacja_P.CurrentFilter1_C *
  ...
  /* TransferFcn: '<S1>/Position Filter1' */
  Lewitacja_B.PositionFilter1 = 0.0;
  Lewitacja_B.PositionFilter1 += Lewitacja_P.PositionFilter1_C *
    Lewitacja_X.PositionFilter1_CSTATE;
  if (rtmIsMajorTimeStep(Lewitacja_M)) {
    /* Level2 S-Function Block: '<S1>/PWM' (ML_PWM) */
    {
      SimStruct *rts = Lewitacja_M->childSfunctions[1];
      sfcnOutputs(rts, 1);
    }

    /* Constant: '<S1>/PWM Brake' */
    Lewitacja_B.PWMBrake = Lewitacja_P.PWMBrake_Value;

    /* Constant: '<S1>/PWM Mode' */
    Lewitacja_B.PWMMode = Lewitacja_P.PWMMode_Value;

    /* Constant: '<S1>/PWM Prescaler' */
...
void Lewitacja_derivatives(void)
{
  ...
  /* Derivatives for TransferFcn: '<S1>/Position Filter' */
  ...
  /* Derivatives for TransferFcn: '<S1>/Velocity Filter' */
  ...
  /* Derivatives for TransferFcn: '<S1>/Current Filter' */
  ...
  /* Derivatives for TransferFcn: '<S1>/Current Filter1' */
  ...
  /* Derivatives for TransferFcn: '<S1>/Position Filter1' */
  _rtXdot->PositionFilter1_CSTATE = 0.0;
  _rtXdot->PositionFilter1_CSTATE += Lewitacja_P.PositionFilter1_A *
    Lewitacja_X.PositionFilter1_CSTATE;
  _rtXdot->PositionFilter1_CSTATE += Lewitacja_B.AnalogInputs[0];
}

/* Model initialize function */
void Lewitacja_initialize(void)
{
  /* InitializeConditions for TransferFcn: '<S1>/Position Filter' */
  Lewitacja_X.PositionFilter_CSTATE = 0.0;

  /* InitializeConditions for TransferFcn: '<S1>/Velocity Filter' */
  Lewitacja_X.VelocityFilter_CSTATE = 0.0;

  /* InitializeConditions for Memory: '<S1>/Memory' */
  Lewitacja_DW.Memory_PreviousInput = Lewitacja_P.Memory_X0;

  /* InitializeConditions for TransferFcn: '<S1>/Current Filter' */
  Lewitacja_X.CurrentFilter_CSTATE = 0.0;

  /* InitializeConditions for TransferFcn: '<S1>/Current Filter1' */
  Lewitacja_X.CurrentFilter1_CSTATE = 0.0;

  /* InitializeConditions for TransferFcn: '<S1>/Position Filter1' */
  Lewitacja_X.PositionFilter1_CSTATE = 0.0;
}
...
RT_MODEL_Lewitacja_T *Lewitacja(void)
{
  ...
  /* External mode info */
  Lewitacja_M->Sizes.checksums[0] = (1360664273U);
  Lewitacja_M->Sizes.checksums[1] = (2268696333U);
  Lewitacja_M->Sizes.checksums[2] = (227861092U);
  Lewitacja_M->Sizes.checksums[3] = (2574047189U);
  ...
  /* register each child */
  {
    ...
    /* Level2 S-Function Block: Lewitacja/<S1>/Analog Inputs (ML_AnalogInputs) */
    {
      SimStruct *rts = Lewitacja_M->childSfunctions[0];
    ...
    /* Level2 S-Function Block: Lewitacja/<S1>/PWM (ML_PWM) */
    {
      SimStruct *rts = Lewitacja_M->childSfunctions[1];
  ...
  Lewitacja_M->Sizes.numBlocks = (23); /* Number of blocks */
  Lewitacja_M->Sizes.numBlockIO = (16);/* Number of block outputs */
  Lewitacja_M->Sizes.numBlockPrms = (98);/* Sum of parameter "widths" */
  return Lewitacja_M;
}
